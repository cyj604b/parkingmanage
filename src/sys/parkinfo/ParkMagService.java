...
public class ParkMagService {
    ...
    public void printStaffInfo(Staff staff)
    {
        ...
        for(int i=0;i<parks.size();i++)
        {
            ...
            System.out.println("停车场编号："+park.getParkNumber());
     ...
     * @return
     */
    public boolean pickUpCar(ArrayList<Park> parks, Car car) {
        // TODO Auto-generated method stub
        boolean tag = false;
        for(Park targetPark:parks)
        {
            targetPark.getAllCars().equals(car);
            tag = targetPark.takeCar(car);
            if (tag)//取到车
            {
                break;
            }
